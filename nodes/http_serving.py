...
class HTTPServing:
    ...
    def http_handler(self):
        class RequestHandler(BaseHTTPRequestHandler):
            def do_POST(self2):
                ...
                self2.end_headers()
                self2.wfile.write(json.dumps({"status": "received"}).encode('utf-8'))
            ...
            def do_GET(self2):
                if self2.path == '/':
                    ...
                    self2.end_headers()
                    self2.wfile.write(b"HTTP Serving is running")
    ...
    def serve(self, port):
        ...
        data["serve_text_function"] = lambda text: json.dumps({"text": text, "_requestId": data["_requestId"]})
